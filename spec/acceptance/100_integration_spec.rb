...
describe 'Atlassian Stash with Embedded Database' do
	include_examples 'an acceptable Stash instance', 'using an embedded database'
end

describe 'Atlassian Stash with PostgreSQL 9.3 Database' do
	include_examples 'an acceptable Stash instance', 'using a PostgreSQL database' do
		if ENV["CI"] == "true"
			before :all do
        ...
        # Create Stash database
        $container_postgres.exec ["psql", "--username", "postgres", "--command", "create database stashdb owner postgres encoding 'utf8';"]
			end
			after :all do
...
describe 'Atlassian Stash with MySQL 5.6 Database', order: :defined do
	include_examples 'an acceptable Stash instance', 'using a MySQL database' do
		if ENV["CI"] == "true"
			before :all do
        ...
        # Create Stash database
        $container_mysql.exec ['mysql', '--user=root', '--password=mysecretpassword', '--execute', 'CREATE DATABASE stashdb CHARACTER SET utf8 COLLATE utf8_bin;']
      end
      after :all do
